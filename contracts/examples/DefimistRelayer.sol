...
contract DefimistRelayer {
    ...
    address public immutable defimistFactory;
    address public immutable defimistRouter;
    ...
    constructor(
        ...
        address _defimistFactory,
        address _defimistRouter,
        ...
        owner = _owner;
        defimistFactory = _defimistFactory;
        defimistRouter = _defimistRouter;
    ...
    function orderLiquidityProvision(
        ...
        require(factory == defimistFactory || factory == uniswapFactory, 'DefimistRelayer: INVALID_FACTORY');
        require(msg.sender == owner, 'DefimistRelayer: CALLER_NOT_OWNER');
        require(tokenA != tokenB, 'DefimistRelayer: INVALID_PAIR');
        require(tokenA < tokenB, 'DefimistRelayer: INVALID_TOKEN_ORDER');
        require(amountA > 0 && amountB > 0, 'DefimistRelayer: INVALID_TOKEN_AMOUNT');
        require(priceTolerance <= PARTS_PER_MILLION, 'DefimistRelayer: INVALID_TOLERANCE');
        require(block.timestamp <= deadline, 'DefimistRelayer: DEADLINE_REACHED');
        require(maxWindowTime > 30, 'DefimistRelayer: INVALID_WINDOWTIME');
        
        if (tokenA == address(0)) {
            require(address(this).balance >= amountA, 'DefimistRelayer: INSUFFICIENT_ETH');
        } else {
            require(IERC20(tokenA).balanceOf(address(this)) >= amountA, 'DefimistRelayer: INSUFFICIENT_TOKEN_A');
        }
        require(IERC20(tokenB).balanceOf(address(this)) >= amountB, 'DefimistRelayer: INSUFFICIENT_TOKEN_B');
        ...
        (uint reserveA, uint reserveB,) = IDefimistPair(pair).getReserves();
    ...
    function orderLiquidityRemoval(
        ...
        require(tokenA != tokenB, 'DefimistRelayer: INVALID_PAIR');
        require(tokenA < tokenB, 'DefimistRelayer: INVALID_TOKEN_ORDER');
        require(amountA > 0 && amountB > 0 && liquidity > 0, 'DefimistRelayer: INVALID_LIQUIDITY_AMOUNT');
        require(priceTolerance <= PARTS_PER_MILLION, 'DefimistRelayer: INVALID_TOLERANCE');
        require(block.timestamp <= deadline, 'DefimistRelayer: DEADLINE_REACHED');
        require(maxWindowTime > 30, 'DefimistRelayer: INVALID_WINDOWTIME');
        ...
        address defimistPair = DefimistLibrary.pairFor(address(defimistFactory), tokenA, tokenB);
        (uint reserveA, uint reserveB,) = IDefimistPair(defimistPair).getReserves();
        uint256 windowTime = _consultOracleParameters(amountA, amountB, reserveA, reserveB, maxWindowTime);
        orders[orderIndex].oracleId = oracleCreator.createOracle(windowTime, pair);
    ...
    function executeOrder(uint256 orderIndex) external {
        ...
        require(orderIndex < orderCount, 'DefimistRelayer: INVALID_ORDER');
        require(!order.executed, 'DefimistRelayer: ORDER_EXECUTED');
        require(oracleCreator.isOracleFinalized(order.oracleId) , 'DefimistRelayer: OBSERVATION_RUNNING');
        require(block.timestamp <= order.deadline, 'DefimistRelayer: DEADLINE_REACHED');
        ...
        amountB = oracleCreator.consult(
          order.oracleId,
          tokenA == address(0) ? IDefimistRouter(defimistRouter).WETH() : tokenA,
          order.amountA 
        );
        ...
        if(order.action == PROVISION){
            ...
            address pair = _pair(tokenA, tokenB, defimistFactory);
            _unpool(
              tokenA, 
              tokenB, 
              pair, 
    ...
    function updateOracle(uint256 orderIndex) external {
        Order storage order = orders[orderIndex];
        require(block.timestamp <= order.deadline, 'DefimistRelayer: DEADLINE_REACHED');
        require(!oracleCreator.isOracleFinalized(order.oracleId) , 'DefimistRelayer: OBSERVATION_ENDED');
        uint256 amountBounty = GAS_ORACLE_UPDATE.mul(tx.gasprice).add(BOUNTY);
        
        (uint reserveA, uint reserveB,) = IDefimistPair(order.oraclePair).getReserves();
        require(
            reserveA >= order.minReserveA && reserveB >= order.minReserveB,
            'DefimistRelayer: RESERVE_TO_LOW'
        );
    ...
    function withdrawExpiredOrder(uint256 orderIndex) external {
        Order storage order = orders[orderIndex];
        require(msg.sender == owner, 'DefimistRelayer: CALLER_NOT_OWNER');
        require(block.timestamp > order.deadline, 'DefimistRelayer: DEADLINE_NOT_REACHED');
        require(order.executed == false, 'DefimistRelayer: ORDER_EXECUTED');
        address tokenA = order.tokenA;
        address tokenB = order.tokenB;
    ...
    function _pool(
        ...
        TransferHelper.safeApprove(_tokenA, defimistRouter, _amountA);
        TransferHelper.safeApprove(_tokenB, defimistRouter, _amountB);
        (amountA, amountB, liquidity) = IDefimistRouter(defimistRouter).addLiquidity(
            _tokenA,
            _tokenB,
            _amountA,
            _amountB,
        ...
        TransferHelper.safeApprove(_tokenA, defimistRouter, 0);
        TransferHelper.safeApprove(_tokenB, defimistRouter, 0);
    }
    ...
    function _unpool(
        ...
        TransferHelper.safeApprove(_pair, defimistRouter, _liquidity);
        (uint amountA, uint amountB) = IDefimistRouter(defimistRouter).removeLiquidity(
            _tokenA,
            _tokenB,
            _liquidity,
            _minA,
        ...
        TransferHelper.safeApprove(_pair, defimistRouter, 0);
        if(_tokenA == WETH){
    ...
    // Internal function to return the correct pair address on either Defimist or Uniswap
    function _pair(address tokenA, address tokenB, address factory) internal view returns (address pair) {
      ...
      pair = IDefimistFactory(factory).getPair(tokenA, tokenB);
    }
